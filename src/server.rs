...
use crate::{bridge, PvxsError, Result, Value, AlarmSeverity, AlarmStatus, AlarmConfig, compute_alarm_for_scalar, AlarmMetadata, DisplayMetadata, ControlMetadata};
...
impl Server {
    ...
    pub fn create_pv_enum(&self, name: &str, choices: Vec<&str>, selected_index: i16, metadata: NTEnumMetadataBuilder) -> Result<()> {
        self.handle.create_pv_enum(name, choices, selected_index, metadata)
    }
    ...
    pub fn stop_drop(mut self) -> Result<()> {
        ...
        self.handle
            ...
            .map_err(|_| PvxsError::new("Server worker stopped"))?;
        let result = reply_rx.recv().map_err(|_| PvxsError::new("Server worker stopped"))?;
    ...
    fn start_inner(isolated: bool) -> Result<Self> {
        ...
        let join = thread::spawn(move || {
            ...
            while let Ok(cmd) = rx.recv() {
                match cmd {
                    ...
                    ManagerCommand::CreateEnum { name, choices, selected_index, metadata, reply } => {
                        let result = if pvs.contains_key(&name) {
                            Err(PvxsError::new("PV already exists"))
                        } else {
                            let choices_refs: Vec<&str> = choices.iter().map(|s| s.as_str()).collect();
                            match server.create_pv_enum(&name, choices_refs, selected_index, metadata) {
                                Ok(pv) => {
        ...
        Ok(Self {
            handle: ServerHandle { tx, tcp_port, udp_port },
            join: Some(join),
        })
    }
}
...
impl SharedPV {
    ...
    pub(crate) fn post_double_with_alarm(&mut self, value: f64, severity: AlarmSeverity, status: AlarmStatus, message: String) -> Result<()> {
        bridge::shared_pv_post_double_with_alarm(self.inner.pin_mut(), value, severity as i32, status as i32, message)?;
        Ok(())
    }

    pub(crate) fn post_int32_with_alarm(&mut self, value: i32, severity: AlarmSeverity, status: AlarmStatus, message: String) -> Result<()> {
        bridge::shared_pv_post_int32_with_alarm(self.inner.pin_mut(), value, severity as i32, status as i32, message)?;
        Ok(())
    }
...
impl NTScalarMetadataBuilder {
    ...
    pub fn alarm(mut self, severity: AlarmSeverity, status: AlarmStatus, message: impl Into<String>) -> Self {
