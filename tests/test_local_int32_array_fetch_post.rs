...
mod test_pvxs_local_int32_array_fetch_post {
    ...
    fn test_pv_local_int32_array_fetch_post() {
        ...
        match loc_srv.fetch_int32_array(name) {
            Ok(fetched) => {
                assert_eq!(fetched.value, initial_array, "Fetched array does not match initial array");
            }
    ...
    fn test_pv_local_int32_array_boundary_values() {
        ...
        match loc_srv.fetch_int32_array(name){
            Ok(fetched) => {
                assert_eq!(fetched.value, boundary_array, "Fetched arrar does not match boundary array");
            }
    ...
    fn test_pv_local_int32_posting_to_array() {
        ...
        loc_srv.post_int32_array(name, initial_array.clone()).expect("Failed to post new int32 array");

        // Fetch and verify only element 0 has changed
        let fetched = loc_srv.fetch_int32_array(name).expect("Failed to fetch after post");
        assert_eq!(fetched.value, initial_array, "Array after post does not match expected values");
    }
}
