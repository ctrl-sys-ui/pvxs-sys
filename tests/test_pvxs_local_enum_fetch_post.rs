...
fn test_pv_local_enum_fetch_post() {
    ...
    let mut srv_pv_loc_enum: SharedPV = loc_srv.create_pv_enum("loc:enum", choices.clone(), initial_index)
...
fn test_pv_local_enum_all_states() {
    ...
    let mut srv_pv_loc_enum: SharedPV = loc_srv.create_pv_enum("loc:enum", choices.clone(), 0)
        .expect("Failed to create pv:enum");
