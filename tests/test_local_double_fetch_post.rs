mod test_pvxs_local_double_fetch_post {
    ...
    fn test_pv_local_double_fetch_post() {
        ...
        loc_srv.create_pv_double(
            name, initial_value, NTScalarMetadataBuilder::new())
        ...
        match loc_srv.post_int32(name, 42) {
            Ok(_) => assert!(false, "Expected error when posting int32 to double pv, but got Ok"),
            Err(e) => assert!(format!("{}", e).contains("type mismatch"), "Expected type mismatch error, but got: {:?}", e),
        }

        // Fetch again to verify the converted value has not changed due to failed post
        match loc_srv.fetch_double(name) {
            Ok(fetched) => assert_eq!(fetched.value, initial_value, "Value mismatch after posting int32"),
            Err(e) => assert!(false, "Failed to fetch value: {:?}", e),
        }
        ...
        // Fetch again to verify the converted value has not changed due to failed post
        match loc_srv.fetch_double(name) {
            Ok(fetched) => assert_eq!(fetched.value, initial_value, "Value mismatch after posting string"),
            Err(e) => assert!(false, "Failed to fetch value: {:?}", e),
        }
        ...
        match loc_srv.fetch_double(name) {
            Ok(fetched) => assert_eq!(fetched.value, new_value, "Value mismatch after posting new double"),
            Err(e) => assert!(false, "Failed to fetch value: {:?}", e),
        }
    }
    ...
    fn test_pv_local_double_fetch_post_with_error_propagation() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert_eq!(fetched_value.value, initial_value, "Value should remain unchanged after failed post");

        // Now post a valid value and verify
        let new_value = 987.654;
        loc_srv.post_double(name, new_value)?;
        let fetched_value = loc_srv.fetch_double(name)?;
        assert_eq!(fetched_value.value, new_value, "Value mismatch after posting new double");
    ...
    fn test_pv_local_double_special_values() {
        ...
        match loc_srv.post_double(name, f64::NAN) {
            Ok(_) => {
                ...
                assert!(loc_srv.fetch_double(name).is_ok(), "NaN value did not post successfully");
            },
            Err(e) => assert!(false, "Server may not support NaN: {:?}", e),
        }

        // Test very large and very small numbers
        loc_srv.post_double(name, f64::MAX).expect("Failed to post max value");
        let value = loc_srv.fetch_double(name).unwrap();
        assert_eq!(value.value, f64::MAX);

        loc_srv.post_double(name, f64::MIN).expect("Failed to post min value");
        let value = loc_srv.fetch_double(name).unwrap();
